...
function projection(
  ...
  const safeContribution = Number.isFinite(contribution)
    ? Math.max(0, contribution)
    : 0
  const safeYears = Number.isFinite(years)
    ? Math.max(0, Math.min(100, years))
    : 0
  const safeReturn = Number.isFinite(annualReturn)
    ? Math.max(0, Math.min(12, annualReturn))
    : 0
  const months = safeYears * 12
  const monthlyRate = safeReturn / 100 / 12
  if (monthlyRate === 0) return safeInitial + safeContribution * months
  return (
    safeInitial * Math.pow(1 + monthlyRate, months) +
    safeContribution * ((Math.pow(1 + monthlyRate, months) - 1) / monthlyRate)
  )
}
...
export default function Investing101() {
  ...
  const calculationError =
    inputs.retirementAge <= inputs.age
      ? 'Retirement age must be greater than current age.'
      : inputs.expectedReturn < 0 || inputs.expectedReturn > 12
        ? 'Expected return must be between 0% and 12%.'
        : inputs.inflation < 0 || inputs.inflation > 10
          ? 'Inflation must be between 0% and 10%.'
          : null
  ...
  const plan = useMemo(() => {
    ...
    const retirementValue = projection(
      startPortfolio,
    ...
    const realValue =
      retirementValue / Math.pow(1 + inputs.inflation / 100, years)
    const yearly = Array.from(
      { length: Math.min(years, 7) + 1 },
      (_, index) => {
        const year = Math.round(
          inputs.age + (years / Math.min(years || 1, 7)) * index,
        )
  ...
  const scenarios = useMemo(() => {
    ...
    return [
      {
        name: 'Invest $50k now',
        value: projection(50000, 0, years, rate),
        tone: 'blue',
      },
      {
        name: 'Spread $50k over 12 months',
        value: projection(0, 50000 / 12, years, rate),
        tone: 'teal',
      },
      { name: 'Keep $50k as cash', value: 50000, tone: 'gray' },
      {
        name: 'Invest $500 / month',
        value: projection(0, 500, years, rate),
        tone: 'indigo',
      },
      {
        name: 'Invest $1,000 / month',
        value: projection(0, 1000, years, rate),
        tone: 'violet',
      },
    ]
  ...
  const updateNumber =
    (key: keyof typeof inputs) => (event: ChangeEvent<HTMLInputElement>) => {
      setInputs((current) => ({
        ...current,
    ...
    <div className={styles.page}>
      <header className={styles.header}>
        <a className={styles.brand} href="#top">
          <span>Investing</span> 101 <b>Canada</b>
        </a>
        ...
        <a className={styles.headerButton} href="#planner">
      ...
      <main id="top">
        <section className={styles.hero}>
          <div className={styles.heroCopy}>
            <p className={styles.eyebrow}>
              <span className={styles.dot} /> A calm place to start
            </p>
            <h1>
              Understand your money.
              <br />
              <em>Then make a plan.</em>
            </h1>
            <p className={styles.lede}>
              Investing 101 Canada is a plain-language guide for understanding
              the choices in front of you, one practical step at a time.
            </p>
            <div className={styles.heroActions}>
              <a className={styles.primaryButton} href="#planner">
                Start with my numbers <span aria-hidden="true">→</span>
              </a>
              <a className={styles.textButton} href="#roadmap">
                Explore the roadmap <span aria-hidden="true">↓</span>
              </a>
            </div>
            <p className={styles.heroNote}>
              <Icon>✦</Icon> Built for Canadians who want clarity, not pressure.
            </p>
          </div>
        ...
        <section className={styles.basics} aria-labelledby="basics-title">
          ...
          <div className={styles.basicsGrid}>
            <article>
              <Icon>↗</Icon>
              <h3>Investing is ownership</h3>
              <p>
                When you invest, your money is usually buying a small piece of
                many businesses, lending to governments, or both.
              </p>
            </article>
            <article>
              <Icon>◌</Icon>
              <h3>Inflation changes what money buys</h3>
              <p>
                Cash is useful for near-term needs. Over long periods, rising
                prices can reduce what that cash can purchase.
              </p>
            </article>
            <article>
              <Icon>+</Icon>
              <h3>Growth can build on growth</h3>
              <p>
                Compounding means returns can begin to earn returns. Time can
                matter as much as the amount you start with.
              </p>
            </article>
            <article>
              <Icon>⇄</Icon>
              <h3>Saving and investing have different jobs</h3>
              <p>
                Savings can protect short-term goals. Investing accepts some
        ...
        >
          <div className={styles.sectionIntro}>
            ...
            <h2 id="roadmap-title">Put your money in the right order.</h2>
            <p>
              There is no universal sequence. This is a practical way to
              organize the questions, not a prescription.
            </p>
          </div>
          <div className={styles.accountGrid}>
            {accounts.map((account, index) => (
              <article className={styles.accountCard} key={account.title}>
        ...
        <section
          id="learn"
          className={styles.etfSection}
          aria-labelledby="etf-title"
        >
          <div className={styles.sectionIntro}>
            <p className={styles.kicker}>ETF education</p>
            <h2 id="etf-title">A simpler way to understand diversification.</h2>
            <p>
              An ETF is a fund that trades on an exchange. Many ETFs hold a
              collection of investments, which can make diversification more
              accessible.
            </p>
          </div>
          <div className={styles.etfLayout}>
            <article className={styles.etfPrimer}>
              <div className={styles.primerIcon}>◎</div>
              <h3>Three ideas to understand first</h3>
              <div>
                <b>Index investing</b>
                <p>
                  Some ETFs aim to follow a market index instead of selecting
                  individual stocks.
                </p>
              </div>
              <div>
                <b>MER fees</b>
                <p>
                  Management expense ratios are ongoing fund costs expressed as
                  a percentage. Small differences can matter over time.
                </p>
              </div>
              <div>
                <b>Risk levels</b>
                <p>
                  More stock exposure can mean bigger swings. Bonds and
                  cash-like holdings can change the balance, not erase risk.
                </p>
              </div>
            </article>
            <div className={styles.etfList}>
              {etfs.map(([name, type, description]) => (
                <article key={name} className={styles.etfCard}>
                  <div>
                    <span>{type}</span>
                    <h3>{name}</h3>
                  </div>
                  <p>{description}</p>
                  <small>Example only, not a recommendation</small>
                </article>
              ))}
            </div>
          </div>
        ...
        >
          <div className={styles.plannerHeading}>
            <div>
              ...
              <h2 id="planner-title">See the moving pieces together.</h2>
              <p>
                Use rough numbers to start a conversation with yourself or a
                qualified professional. Nothing is saved.
              </p>
            </div>
            <div
              className={styles.plannerTabs}
              role="tablist"
            ...
            >
              <button
                ...
                className={activeTab === 'plan' ? styles.activeTab : ''}
              ...
              <button
                id="planner-tab-whatif"
                type="button"
                aria-controls="planner-panel"
                tabIndex={activeTab === 'whatif' ? 0 : -1}
                className={activeTab === 'whatif' ? styles.activeTab : ''}
          ...
          {!acknowledged && (
            <div className={styles.calculatorGate}>
              <div>
                <Icon>✓</Icon>
                <h3>One quick acknowledgement</h3>
                <p>
                  Before using the calculator, please confirm that you
                  understand this is an educational model, not personal
                  financial advice.
                </p>
                <label>
                  <input
                  ...
                  />{' '}
                  I understand and want to explore a planning scenario.
                </label>
              </div>
            </div>
          )}
          {acknowledged && (
            <div
              className={styles.calculatorContent}
              id="planner-panel"
              role="tabpanel"
              aria-labelledby={
                activeTab === 'plan' ? 'planner-tab-plan' : 'planner-tab-whatif'
              }
            >
              {activeTab === 'plan' ? (
                <>
                  ...
                  >
                    ...
                    <div className={styles.fields}>
                      ...
                      <Field
                        ...
                        onChange={updateNumber('retirementAge')}
                        suffix="years"
                        min={inputs.age + 1}
                        max={100}
                      />
                      <Field
                        ...
                        onChange={updateNumber('monthlyExpenses')}
                        prefix="$"
                        max={100000000}
                      />
                      <Field
                        ...
                        onChange={updateNumber('monthlyRent')}
                        prefix="$"
                        max={100000000}
                      />
                      <Field
                        ...
                        value={inputs.cash}
                        onChange={updateNumber('cash')}
                        prefix="$"
                        max={100000000}
                      />
                      <Field
                        label="House sale proceeds"
                        value={inputs.houseProceeds}
                        onChange={updateNumber('houseProceeds')}
                        prefix="$"
                        max={100000000}
                      />
                      <Field
                        ...
                        onChange={updateNumber('existingInvestments')}
                        prefix="$"
                        max={100000000}
                      />
                      <Field
                        label="Upcoming large expenses"
                        value={inputs.upcomingExpenses}
                        onChange={updateNumber('upcomingExpenses')}
                        prefix="$"
                        max={100000000}
                      />
                      <Field
                        ...
                        onChange={updateNumber('monthlyContribution')}
                        prefix="$"
                        max={1000000}
                      />
                      <Field
                        ...
                        onChange={updateNumber('expectedReturn')}
                        suffix="%"
                        step="0.5"
                        max={12}
                      />
                      <Field
                        ...
                        onChange={updateNumber('inflation')}
                        suffix="%"
                        step="0.5"
                        max={10}
                      />
                      <label>
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setInputs((current) => ({
                              ...current,
                              emergencyMonths: Number(event.target.value),
                            }))
                          }
                        >
                    ...
                    <p className={styles.fieldFootnote}>
                      TFSA and RRSP room are personal. Confirm available room
                      with the CRA before contributing.
                    </p>
                  </form>
                  <div className={styles.resultsPanel} aria-live="polite">
                    ...
                    <div className={styles.resultGrid}>
                      <Result
                        ...
                        value={money(plan.emergencyFund)}
                        detail={`${inputs.emergencyMonths} months of core costs`}
                      />
                      <Result
                        label="Cash reserve"
                        value={money(plan.cashReserve)}
                        detail="Emergency fund + known expenses"
                      />
                      <Result
                        label="Potentially investable"
                        value={money(plan.investable)}
                        detail="After the reserve in this model"
                      />
                      <Result
                        label="Monthly investing"
                        value={money(inputs.monthlyContribution)}
                        detail="Your stated contribution"
                      />
                    </div>
                    {calculationError ? (
                      ...
                      <>
                        <article className={styles.retirementCard}>
                          <div>
                            <p>
                              Illustrative value at age {inputs.retirementAge}
                            </p>
                            <h3>{money(plan.retirementValue)}</h3>
                            <span>
                              About {money(plan.realValue)} in today&apos;s
                              dollars using your inflation assumption.
                            </span>
                          </div>
                          <div className={styles.retirementMeta}>
                            <b>{plan.years} years</b>
                            <span>to retirement</span>
                            <b>{money((plan.retirementValue * 0.04) / 12)}</b>
                            <span>
                              monthly at a hypothetical 4% annual withdrawal
                            </span>
                          </div>
                        </article>
                        <div
                        ...
                        >
                          <div className={styles.chartTitle}>
                            <span>Illustrative growth path</span>
                            <small>
                              Based on your return and contribution assumptions
                            </small>
                          </div>
                          <div className={styles.chartBars}>
                            {plan.yearly.map((item) => (
                              <div className={styles.barGroup} key={item.year}>
                                <div
                                  className={styles.bar}
                                  style={{
                                    height: `${Math.max(8, (item.value / chartMax) * 100)}%`,
                                  }}
                                >
                ...
                <div className={styles.whatIf}>
                  <div className={styles.scenarioCopy}>
                    <p className={styles.resultLabel}>Compare a few paths</p>
                    <h3>
                    ...
                    <p>
                      This illustration uses {inputs.expectedReturn}% annual
                      growth over{' '}
                      {Math.max(
                        1,
                        Math.min(20, inputs.retirementAge - inputs.age),
                      )}{' '}
                      years. It cannot predict market returns.
                    </p>
                    <label>
                      ...
                      <input
                        ...
                        onChange={updateNumber('expectedReturn')}
                      />
                      <b>{inputs.expectedReturn}%</b>
                    </label>
                  </div>
                  <div className={styles.scenarioChart}>
                    {scenarios.map((scenario) => (
                      <div className={styles.scenarioRow} key={scenario.name}>
                        <div>
                          <span>{scenario.name}</span>
                          <b>{money(scenario.value)}</b>
                        </div>
                        <div className={styles.scenarioTrack}>
                          <span
                            className={`${styles.scenarioBar} ${styles[scenario.tone]}`}
                            style={{
                              width: `${(scenario.value / scenarioMax) * 100}%`,
                            }}
                          />
                        </div>
                      </div>
        ...
        <section
          id="brokers"
          className={styles.brokers}
          aria-labelledby="broker-title"
        >
          <div className={styles.sectionIntro}>
            <p className={styles.kicker}>Broker comparison</p>
            <h2 id="broker-title">A brokerage is a tool, not a verdict.</h2>
            <p>
              There is no universally best platform. Compare what matters for
              your habits, account type, and the services you actually want.
              Fees and features change, so use the current published schedule
              before deciding.
            </p>
          </div>
          <div className={styles.brokerTable}>
            <div className={styles.brokerHead}>
              <span>Broker</span>
              <span>What people often value</span>
              <span>May suit</span>
              <span>Check before deciding</span>
            </div>
            {brokers.map(([name, value, fit, check]) => (
              <article key={name}>
                <h3>{name}</h3>
                <p>{value}</p>
                <p>{fit}</p>
        ...
        <section className={styles.guides} aria-labelledby="guide-title">
          <div className={styles.guideCard}>
            <p className={styles.kicker}>Step-by-step checklist</p>
            <h2 id="guide-title">
              Opening an account should not feel mysterious.
            </h2>
            <p>
              This checklist outlines a typical process: choose an account type,
              complete identity checks, link and fund an account, understand the
              order, and review before submitting.
            </p>
            ...
            <a className={styles.primaryButton} href="#planner">
              Use the planning tool <span aria-hidden="true">→</span>
            </a>
          </div>
        </section>
        ...
        <section className={styles.library} aria-labelledby="library-title">
          <div className={styles.sectionIntro}>
            <p className={styles.kicker}>Educational library</p>
            <h2 id="library-title">Topics to explore next.</h2>
            <p>
              These are suggested research topics, not links to unfinished
              articles.
            </p>
          </div>
      ...
      <footer className={styles.footer}>
        ...
        <p>{disclaimer}</p>
        <span>© 2026 Investing 101 Canada · Educational planning only</span>
      </footer>
    </div>
  )
}
